...
def download_access_stats_old(
    ...
    country_base_url = (
        f"https://irus.jisc.ac.uk/IRUSConsult/irus-oapen/v2/br1bCountry/?frmRepository=1%7COAPEN"
...
def download_access_stats_new(
    ...
    for base_item, ip_item, country_item in zip(
...
def replace_ip_address(
    client_ip: str, geoip_client: geoip2.database.Reader
...
def download_file_from_storage_bucket(file_path: str, bucket_name: str, blob_name: str) -> bool:
    ...
    :param bucket_name: The storage bucket name
    :param blob_name: The blob name inside the storage bucket
    :return: Whether file at local file path exists.
    """
    storage_client = storage.Client()
    bucket = storage_client.bucket(bucket_name)
    blob = bucket.blob(blob_name)
